C
     ...
     3     AILCG,  TAIR,   TCAN,   TBAR,       THLIQ,      THPOR,        & !HSuo, Jan2013, AILC-->AILCG
...
C         AILCG        - LAI OF TOTAL LEAF, 9PFTs  !HSuo, Jan2013
C         AILCB        - LAI OF BROWN LEAF, 9PFTs  !HSuo, Jan2013
...
C
C         CLEAF       - TOTAL EAF BIOMASS C [kgC/m2] 
...
C         NRUB         - LEAVE RUBISCO-RELATED N CONTENT [gC/m2 COVERED SURFACE AREA]
C         NRUB0        - LEAVE RUBISCO-RELATED N CONTENT AT TOP CANOPY [gC/m2 LEAF AREA]
C         NLITR_HS     - LITTER N CONTENT [gN/m2]
C         NSOM_HS      - SOM N CONTENT [gN/m2]
C 
...
C
     ...
     1 AILCG(ILG, ICC)  !HSuo, Nov2012, AILC-->AILCG
C
      REAL TAIR(ILG),         TCAN(ILG)   
      REAL TBAR(ILG, IG),    THLIQ(ILG, IG),    THPOR(ILG, IG)      !HSuo
C     REAL TBAR(ILG, ILG),    THLIQ(ILG, ILG),    THPOR(ILG, ILG)
...
C
	...
	REAL GROWDAYS,  FNH4UP
	REAL DNPUPALL,  DNPLTRALL,  DNBFIXALL
C-------------------------------------------------------------------------HSuo ADD 
      REAL NLITR_HS(ILG,ICC+1),NSOM_HS(ILG,ICC+1), N2OTOT(ILG,ICC+1),   !HSuo N2O flux, in g N ha-1 d-1
     1     N2TOT(ILG,ICC+1)
C-------------------------------------------------------------------------HSuo ADD 
C 
C
...
!---------Glossary---------------
       !FLXdNO3 (g N ha-1 d-1)  the maximum total N gas flux for a given soil NO3 level (assuming high respiration rates)
       !FLXdCO2 (g N ha-1 d-1)  the maximum total N gas flux for a given soil respiration level (assuming high NO3 rates)
       !FLXdWFPS  the effect of WFPS on the denitrification rate (fraction)
!---------Glossary---------------
...
C        SOCRES(I)
       ...
       REAL NSNH4(ILG,ICC), NSNO3(ILG,ICC)! unit convert (gN/m2-->ug N/g soil)
	...
	REAL test1, test2, test3, test4
	 ...
	 !*************parameters from (Parton,1992)*****************
       ...
       Da= 0.55   !4.82  SANDY !4.82  !MEDIUM !60.0 FINE !this better read from the INI
       Db= 1.70   !14.0  SANDY !14.0  !MEDIUM !18.0 FINE
       Dc= -0.007 !16.0  SANDY !16.0  !MEDIUM !22.0 FINE
       Dd= 3.22   !1.39  SANDY !1.39  !MEDIUM !1.06 FINE
	 !*************parameters from (Parton,1992)*****************
...
C
              FNRUB   = (0.31+3.43*EXP(-1.81*AILCG(I,J)))                &  !HSuo Jan2013
...
C
              IF (AILCG(I,J) .GE. LAI0) THEN   !HSuo, Jan2013
                  NRUB0(I,J) = MAX(NRUB(I,J),0.1)
     1                              *((1.0-EXP(-KRUBN*LAI0))/LAI0)      &
     2                               /(1.0-EXP(-KRUBN*AILCG(I,J)))  !HSuo Jan2013
              ELSE
...
C
              ...
              DNNIT(I,J) = MIN(0.99/DELT,KNIT) * SNH4(I,J) 
...
C
      IF (KMIN4 .GT. 0.0 .AND. KMIN3 .GT. 0.0) THEN  !!!HSuo, NOV,2012
              FNH4UP    = KMIN4*SNH4(I,J)/(KMIN4*SNH4(I,J)
     1                                   +KMIN3*SNO3(I,J))
      ENDIF
     ...
     4                        - DNPUPALL*FNH4UP*DELT                    &  !NH4 uptake 
...
C
     ...
     3                        - DNPUPALL*(1.0-FNH4UP)*DELT              &  ! NO3 uptake 
