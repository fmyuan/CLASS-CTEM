      SUBROUTINE SNINFL(R,TR,ZSNOW,TSNOW,RHOSNO,HCPSNO,HTCS,HMFN,PCPG,
     1                  ROFN,FCT,ILG,IL1,IL2,JL)                      
C
...
C
     ...
     1     RHOSNO(ILG),    HCPSNO(ILG),    HTCS  (ILG),    HMFN  (ILG),
     2     PCPG  (ILG),    ROFN  (ILG)
C
C     * INPUT ARRAY.
C
...
C-----------------------------------------------------------------------
      DO 100 I=IL1,IL2
          IF(FCT(I).GT.0. .AND. R(I).GT.0. .AND. ZSNOW(I).GT.0.)
     1                                                              THEN    
     ...
     1                ZSNOW(I)/DELT
              ...
              IF(HRCOOL.GE.(HSNWRM+HSNMLT))                 THEN                                          
                  ...
                  HMFN(I)=HMFN(I)+FCT(I)*CLHMLT*ZMELT*RHOW/DELT
                  HTCS(I)=HTCS(I)+FCT(I)*CLHMLT*ZMELT*RHOW/DELT
                  TR(I)=HRCOOL/(HCPW*(ZMELT+RAIN))                                           
                  R(I)=R(I)+ZMELT/DELT                                                          
                  ZSNOW(I)=0.0                                                               
                  TSNOW(I)=0.0                                                               
                  RHOSNO(I)=0.0                                                              
     ...
     1                                                      THEN
                  ...
                  HMFN(I)=HMFN(I)+FCT(I)*CLHMLT*ZMELT*RHOSNO(I)/DELT
                  HTCS(I)=HTCS(I)+FCT(I)*CLHMLT*ZMELT*RHOSNO(I)/DELT
                  ZSNOW(I)=ZSNOW(I)-ZMELT                                                       
                  ZMELT=ZMELT*RHOSNO(I)/RHOW                                                 
                  TSNOW(I)=0.0                                                               
                  TR(I)=0.0                                                                  
                  R(I)=R(I)+ZMELT/DELT                                                          
              ELSE IF(HSNWRM.GE.(HRCOOL+HRFREZ))            THEN                                      
                  HSNWRM=(HRCOOL+HRFREZ)-HSNWRM                                           
                  HMFN(I)=HMFN(I)-FCT(I)*HRFREZ/DELT
                  HTCS(I)=HTCS(I)-FCT(I)*HRFREZ/DELT
     ...
     1                                                      THEN
                  ...
                  HMFN(I)=HMFN(I)-FCT(I)*CLHMLT*ZFREZ*RHOW/DELT
                  HTCS(I)=HTCS(I)-FCT(I)*CLHMLT*ZFREZ*RHOW/DELT
              ...
              HTCS(I)=HTCS(I)+FCT(I)*HCPSNO(I)*(TSNOW(I)+TFREZ)*
     1                ZSNOW(I)/DELT
              PCPG(I)=PCPG(I)+FCT(I)*R(I)*RHOW
              ROFN(I)=ROFN(I)+FCT(I)*R(I)*RHOW
          ENDIF
  100 CONTINUE
C                                                                          
      RETURN                                                                      
